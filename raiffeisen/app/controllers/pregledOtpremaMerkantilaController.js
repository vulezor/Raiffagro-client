(function(){
	var pregledOtpremaMerkantilaController = function($scope, infoboxService, mineFactory, otpremaMerkantilaPregledFactory, errorService, mainService){
	    $scope.search_data = {};
		...
		$scope.resetPretraga = function(){ //session_id=87582f36f65bd99c137cc5a51b3e7c9a&type_of_goods_id=1&goods_id=94&client_id=1&datum_od=01.01.2016&datum_do=01.01.2016
			...
			$scope.$broadcast('reset_search_table');
		}
		
		//-------------------------------------------------------------------------------------------------------------
		
		$scope.getSearchGoodType = function(){
			otpremaMerkantilaPregledFactory.get_search_good_type($scope.search_data).success(function(msg){
		...
		$scope.getSearchOtpremaData = function(){
		...
		$scope.getTotalOfResult = function(){
			otpremaMerkantilaPregledFactory.get_search_otprema_total($scope.search_data).success(function(msg){
				$('.ajax_load_visibility').css({'visibility':'visible'});
				if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
					$('.ajax_load_visibility').css({'visibility':'hidden'});
					console.log(msg);
		...
		$scope.getExcell = function(){
			console.log($scope.link)
			window.open(mainService.domainURL()+'pregled_otpreme_merkantile_api/getExcell?'+$scope.link,'_blank');
		}
		
	};
	pregledOtpremaMerkantilaController.$inject = ['$scope', 'infoboxService', 'mineFactory', 'otpremaMerkantilaPregledFactory', 'errorService', 'mainService'];
    angular.module('_raiffisenApp').controller('pregledOtpremaMerkantilaController', pregledOtpremaMerkantilaController);
	
	//***************************************************************************************************************************************************************
	
	var pregledOtpremeTable = function( DTOptionsBuilder, DTColumnBuilder, usersFactory, $scope, $resource, $compile, mainService, otpremaMerkantilaPregledFactory, errorService) {
        var vm = this;
       ...
       // vm.stateChange = stateChange;
		...
		$scope.$on("reset_search_table", function(event) {
        ...
        vm.dtOptions = DTOptionsBuilder
            .fromSource(mainService.domainURL()+'pregled_otpreme_merkantile_api/empty_load')
        ...
        function reloadData() {
            ...
            vm.dtInstance.changeData(mainService.domainURL()+'pregled_otpreme_merkantile_api/get_search_otprema/'+vm.result+'/?'+$scope.$parent.link);
        }
		...
		function stornirajPrijemnicu(id){
            console.log(id);
			prijemOtpremaPregledFactory.storniraj_dokument(id).success(function(msg){
				console.log(msg);
			}).error(function(){
				console.log(error);
			});
        ...
        function actionsHtml (data, type, full, meta) {
           vm.prijemResult[data.input_id] = data;
            return'  <i class="fa fa fa-file btn btn-primary btn-sm" style="margin-top:0px" title="Otpremnica" data-ng-click="otpremaShowCase.printPrijemnica(otpremaShowCase.prijemResult[' + data.input_id + '])"></i><!--<i class="fa fa fa-file btn btn-primary btn-sm" style="margin-top:0px" title="Otpremnica" data-ng-click="otpremaShowCase.stornirajPrijemnicu(otpremaShowCase.prijemResult[' + data.input_id + '])"></i>-->';
        }
    };
    pregledOtpremeTable.$inject = ['DTOptionsBuilder', 'DTColumnBuilder', 'usersFactory', '$scope', '$resource', '$compile', 'mainService', 'otpremaMerkantilaPregledFactory', 'errorService'];
    angular.module('_raiffisenApp').controller('pregledOtpremeTable', pregledOtpremeTable);
}());