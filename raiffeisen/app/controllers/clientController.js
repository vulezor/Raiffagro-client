(function(){
    var clientController = function($scope, $filter, infoboxService, placesFactory, wearehouseFactory, clientsFactory, errorService, mainService){
        $scope.clientdata = {};
        $scope.new_places = {};
       ...
       // $scope.formVisibility = $cookies.get('formVisibility') || 'open';
        ...
        $scope.addNewClient = function(){
			...
			if(!$scope.clientdata.hasOwnProperty('selectedPlaceId') ||  $scope.clientdata.selectedPlaceId===0 ||  $scope.clientdata.selectedPlaceId===null || typeof $scope.clientdata.selectedPlaceId === 'undefined'){
                errorService.error_msg($('select[name="selectedPlaceId"]'), "Morate zabrati mesto kupca/dobavljača"); return false;
            }
    ...
    clientController.$inject = ['$scope', '$filter', 'infoboxService', 'placesFactory', 'wearehouseFactory', 'clientsFactory', 'errorService', 'mainService'];
    angular.module('_raiffisenApp').controller('clientController', clientController);


    var clientList = function( DTOptionsBuilder, DTColumnBuilder, clientsFactory, $scope, $resource, $compile, mainService) {
        var vm = this;
        ...
        vm.dtOptions = DTOptionsBuilder
            .fromSource(mainService.domainURL()+'client_api/get_clients')
        ...
        function reloadData() {
            ...
            vm.dtInstance.changeData(mainService.domainURL()+'client_api/get_clients');
            //stateChange()
        }
    ...
    clientList.$inject = ['DTOptionsBuilder', 'DTColumnBuilder', 'clientsFactory', '$scope', '$resource', '$compile', 'mainService'];
