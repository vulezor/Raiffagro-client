(function(){
	var pregledPrijemRepromaterijalController = function($scope, infoboxService, mineFactory, prijemRepromaterijalPregledFactory, errorService, mainService){
	    $scope.search_data = {};
		...
		$scope.getSearchGoodType = function(){
			prijemRepromaterijalPregledFactory.get_search_good_type($scope.search_data).success(function(msg){
		...
		$scope.getSearchGoodName = function(){
			$scope.search_data.lot_id = null;
			prijemRepromaterijalPregledFactory.get_search_good_name($scope.search_data).success(function(msg){
				$('.ajax_load_visibility').css({'visibility':'visible'});
				if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
					$('.ajax_load_visibility').css({'visibility':'hidden'});
					console.log(msg);
		...
		$scope.getGoodClient = function(){
			prijemRepromaterijalPregledFactory.get_search_good_client($scope.search_data).success(function(msg){
				$('.ajax_load_visibility').css({'visibility':'visible'});
				if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
					$('.ajax_load_visibility').css({'visibility':'hidden'});
					console.log(msg);
					$scope.get_search_good_client = msg.clients;
					$scope.get_search_good_lots = msg.lots;
				} else {
					//not loged in
					$scope.$parent.logoutUser();
				}	
		...
		$scope.getTotalOfResult = function(){
			prijemRepromaterijalPregledFactory.get_search_prijem_total($scope.search_data).success(function(msg){
				$('.ajax_load_visibility').css({'visibility':'visible'});
				if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
					$('.ajax_load_visibility').css({'visibility':'hidden'});
					console.log(msg);
		...
		$scope.getExcell = function(){
			...
			window.open(mainService.domainURL()+'pregled_prijema_repromaterijal_api/getExcell?'+$scope.link,'_blank');
		}
	...
	pregledPrijemRepromaterijalController.$inject = ['$scope', 'infoboxService', 'mineFactory', 'prijemRepromaterijalPregledFactory', 'errorService', 'mainService'];
    angular.module('_raiffisenApp').controller('pregledPrijemRepromaterijalController', pregledPrijemRepromaterijalController);
	
	//***************************************************************************************************************************************************************
	
	var pregledPrijemaRepromaterijalTable = function( DTOptionsBuilder, DTColumnBuilder, usersFactory, $scope, $timeout, $resource, $compile, mainService, prijemRepromaterijalPregledFactory, errorService) {
        var vm = this;
       ...
       // vm.stateChange = stateChange;
        ...
        vm.dtOptions = DTOptionsBuilder
            .fromSource(mainService.domainURL()+'pregled_prijema_repromaterijal_api/empty_load')
        ...
        function reloadData() {
            ...
            vm.dtInstance.changeData(mainService.domainURL()+'pregled_prijema_repromaterijal_api/get_search_prijem/'+vm.result+'/?'+$scope.$parent.link);
        }
		
		 function resetData() {
            var resetPaging = false;
            vm.dtInstance.changeData(mainService.domainURL()+'pregled_prijema_repromaterijal_api/empty_load');
        }
        ...
        function printPrijemnica(id){
			...
			prijemRepromaterijalPregledFactory.getPrijem({'session_id':vm.result,'input_id':id.input_id}).success(function(msg){
				console.log(msg);
		...
		function stornirajPrijemnicu(id){
            console.log(id);
			prijemRepromaterijalPregledFactory.storniraj_dokument(id).success(function(msg){
				console.log(msg);
			}).error(function(){
				console.log(error);
			});
    ...
    pregledPrijemaRepromaterijalTable.$inject = ['DTOptionsBuilder', 'DTColumnBuilder', 'usersFactory', '$scope', '$timeout','$resource', '$compile', 'mainService', 'prijemRepromaterijalPregledFactory', 'errorService'];
    angular.module('_raiffisenApp').controller('pregledPrijemaRepromaterijalTable', pregledPrijemaRepromaterijalTable);
}());